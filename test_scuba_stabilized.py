...
#from utils_optical_flow_test_allFrames import DataLoader
...
def main():
    ...
    for iteration in range(sample_args.iteration):
        ...
        model_save_name = save_tar_name+str(sample_args.epoch)+'_seqLen_' + str(120)
        ...
        for batch in range(dataloader.num_batches):
            ...
            # Vector
            #vector = predictionToVector(ret_x_seq.cpu(), uncertainties.cpu(), obsLen=sample_args.obs_length)
            ...
            if dataset_pointer_ins is not dataloader.dataset_pointer:
                if dataloader.dataset_pointer is not 0:
                    ...
                    #iteration_vectors.append(vectors)
        ...
        f = open('inference_time_results.txt','a')
        f.write("Inference Time results for Stabilized "+method_name +'\n')
        f.write("Average Full Forward Pass: %.3f\n"%(averageFullTime))
        f.write("Average Model Only Forward Pass: %.3f\n\n"%(averageModelTime))
        f.close()
        ...
        #vectors_store.append(iteration_vectors)
    ...
    #dataloader.write_to_file(vectors_store[smallest_err_iter_num], result_directory, prefix, model_name)
...
def vectors_preprocess(dataloader, ret_x_seq, pred_length, obs_length, target_id, uncertainties):
    seq_lenght = pred_length + obs_length
    
    #begin and end index of obs. frames in this seq.
    end_obs = (dataloader.frame_pointer - pred_length)
    repeated_id = np.repeat(target_id, 1) # add id
    ret_x_seq_c = ret_x_seq.copy()
    id_integrated_prediction = np.append(repeated_id[:, None], ret_x_seq_c, axis=1)
    frame_integrated_prediction = np.append(frame_number_predicted[:,None], id_integrated_prediction, axis=1) #add frame number
    uncertainty_integrated_prediction = np.append(frame_integrated_prediction,uncertainties, axis=1)
