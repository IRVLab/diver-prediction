...
class DataLoader():
    ...
    def __init__(self,f_prefix, batch_size=4, seq_length=10, num_of_validation = 0, forcePreProcess=False, infer=False, generate = False):
        ...
        # base test files
        base_test_dataset=  [ '/data/stabilized/test/IRVLab/pool_swimmer_004_A.txt'#pool_flipper_001_A.txt'#, '/data/test/IRVLab/pool_flipper_003_A_0009.txt'#/data/test/biwi/biwi_eth.txt',
        ...
        #base train files
        base_train_dataset = ['/data/stabilized/train/IRVLab/pool_swimmer_001_A.txt'#pool_flipper_003_A_0001.txt'#, '/data/test/IRVLab/pool_flipper_003_A_0004.txt'#/data/train/biwi/biwi_hotel.txt',
        ...
        self.base_train_path = 'data/stabilized/train/'
        self.base_test_path = 'data/stabilized/test/'
        self.base_validation_path = 'data/stabilized/valid/'
    ...
    def adjustFramesForOpticalFlow(self, seqFrames):
        """
        seqFrames: Shape (sequenceLength+1, numDivers, 7)
        return transformed frames (sequenceLength+1, numDivers,5)
        """
        orig_boxes = seqFrames[:,:,0:4]
        # Ignore the transform of the first frame since we don't care about it's transform with the previous frame
        # Need only 0-th index of numDivers, since the transform for each diver is the same
        transforms = np.reshape(seqFrames[1:,0,4:], (orig_boxes.shape[0]-1,2))
        return self.transformedBoxes(orig_boxes, transforms)
