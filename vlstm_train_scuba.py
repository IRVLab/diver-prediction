...
def train(args):
    ...
    def checkpoint_path(x):
        return os.path.join(save_directory, method_name, model_name, save_tar_name+str(x)+'.tar')#'_unstabilized.tar')
    ...
    for epoch in range(args.num_epochs):
        ...
        # For each batch
        #print("NUM_BATCHES",dataloader.num_batches)
        for batch in range(dataloader.num_batches):
            ...
            for sequence in range(dataloader.batch_size):
                #print("In train func, working on count ",count)
        ...
        if dataloader.valid_num_batches > 0:
            ...
            if dataloader.valid_num_batches != 0:
                ...
                IoU_epoch /= dataloader.num_batches
                ...
                print('Best epoch', best_epoch_val, 'Best validation loss', best_val_loss, 'Best error epoch',best_err_epoch_val, 'Best error', smallest_err_val, 'Best IoU',bestIoU, 'BestIoU epoch',best_IoU_epoch)
        ...
        if dataloader.additional_validation and (epoch) in validation_epoch_list:
            ...
            for batch in range(dataloader.num_batches):
                ...
                # For each sequence
                for sequence in range(dataloader.batch_size):
                    #print("in sequence %d of %d"%(sequence+1,dataloader.batch_size))
                    # Get data corresponding to the current sequence
                    x_seq ,_ , d_seq, numPedsList_seq, PedsList_seq = x[sequence], y[sequence], d[sequence], numPedsList[sequence], PedsList[sequence]
                    #target_id = target_ids[sequence]
                    ...
                    ret_x_seq, loss = sample_validation_data_vanilla(x_seq[:-1], PedsList_seq[:-1], args, net, lookup_seq, numPedsList_seq, dataloader)
                    
                    if args.use_cuda:
                        ret_x_seq = ret_x_seq.cuda()
                        #first_values_dict = first_values_dict.cuda()
                    #revert the points back to original space
                    ret_x_seq = revert_seq(ret_x_seq, PedsList_seq, lookup_seq, first_values_dict)


                    err, IoU = get_mean_error(ret_x_seq.data, orig_x_seq.data, PedsList_seq, PedsList_seq, args.use_cuda, lookup_seq)
                    f_err, fIoU = get_final_error(ret_x_seq.data, orig_x_seq.data, PedsList_seq, PedsList_seq, lookup_seq)
                ...
                IoU_batch = IoU_batch / dataloader.batch_size
                fIoU_batch = fIoU_batch / dataloader.batch_size
            ...
            if dataloader.num_batches != 0:
                ...
                IoU_epoch /= dataloader.num_batches
                fIoU_epoch /= dataloader.num_batches
                ...
                print('(epoch {}), valid_loss = {:.3f}, valid_mean_err = {:.3f}, valid_final_err = {:.3f} , meanIoU = {:.3f}, finalIoU = {:.3f}'.format(epoch, loss_epoch, err_epoch, f_err_epoch,IoU_epoch, fIoU_epoch))
                print('Best epoch', best_epoch_val_data, 'Best validation loss', best_val_data_loss, 'Best error epoch',best_err_epoch_val_data, 'Best error', smallest_err_val_data, 'BestIoU',bestIoU, 'Best IoU epoch',best_IoU_epoch)
                log_file_curve.write("Validation dataset epoch: "+str(epoch)+" loss: "+str(loss_epoch)+" mean_err: "+str(err_epoch)+'final_err: '+str(f_err_epoch)+'bestIoU: '+str(bestIoU)+'\n')
    ...
    if dataloader.valid_num_batches != 0:
        print('Best epoch', best_epoch_val, 'Best validation Loss', best_val_loss, 'Best error epoch',best_err_epoch_val, 'Best error', smallest_err_val, 'Best IoU', bestIoU, 'BestIoU epoch',bestIoU_epoch)
    ...
    if validation_dataset_executed:
        ...
        print("Skipping writing plot files. Not needed for diver prediction")
