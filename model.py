...
class SocialModel(nn.Module):
    ...
    def getSocialTensor(self, grid, hidden_states):
        ...
        for node in range(numNodes):
            ...
            #print(np.shape(torch.t(grid[node])),np.shape(hidden_states))
            # if np.shape(torch.t(grid[node]))[1] != np.shape(hidden_states)[0]:
            #     print("##############################################")
            #     print(np.shape(torch.t(grid[node])),np.shape(hidden_states))
    ...
    def forward(self, *args):
        ...
        PedsList = Variable(torch.LongTensor(args[4])).cuda()
        num_pedlist = Variable(torch.LongTensor(args[5])).cuda()
        self.dataloader = args[6]
        look_up = args[7]
        ...
        for framenum,frame in enumerate(input_data):
            ...
            # print("CUDA STAT")
            # print("CURR HS: ",hidden_states_current.is_cuda)
            # print("CURR NODES:", nodes_current.is_cuda)
            # print("CURR GRID:", grid_current.is_cuda)

            if not self.gru:
                cell_states_current = torch.index_select(cell_states, 0, corr_index)

            #print("PedsList",PedsList[framenum])
            # Compute the social tensor
        ...
        #reshape_begin = time.time()
        outputs_return = Variable(torch.tensor(np.reshape(outputs.cpu().numpy(), (self.seq_length,numNodes,10))))
        for framenum in range(self.seq_length):
            for node in range(numNodes):
                if not np.allclose(outputs_return[framenum, node, :].cpu().numpy() , outputs[framenum*numNodes + node, :].cpu().numpy()):
                    print("deva")
                    return
        # for framenum in range(self.seq_length):
        #     for node in range(numNodes):
        #         if not (np.allclose(outputs_return[framenum, node, :], outputs[framenum*numNodes + node, :].cpu().numpy())):
        #             print("ohno")
        #             return
        ...
        #tot_end = time.time()
        #tot_time = tot_end - tot_init
        # print("Preprocess time:", (preprocess_end - preprocess_start)/tot_time)
        # print("Model pre:", (modelPre_end - modelPre_start)/tot_time)
        # print("Model time:", (model_end - model_init)/tot_time)
        # print("Model Post:", (modelPost_end-modelPost_begin)/tot_time)
        # print("Reshape:", (reshape_end - reshape_begin)/tot_time)
